...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, char *screen_class, float hier_thresh)
{
	...
	/*  count = count_detections(im, l.w*l.h*l.n, thresh, probs, names, l.classes, "person"); */
        ...
        // Slice the path to identify the name
        const char delimit[2] = "/";
        char *slice;
        char *filename_only;
        slice = strtok(filename, delimit);
        while (slice != NULL) {
            filename_only = slice;
            slice = strtok(NULL, delimit);
        }

        // Remove existing extension
        const char period[2] = ".";
        char *filename_noext;
        filename_noext = strtok(filename_only, period);
...
void count_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *search_name)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
